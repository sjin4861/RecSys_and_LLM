...
from recwizard.utility import loadJsonFileFromDataset, pad_and_stack
...
class RedialDataProcessor:
    ...
    def _replace_movies_in_tokenized(self, tokenized):
        ...
        while index < len(tokenized):
            ...
            if pattern.match(word) and int(word) in self.db2id:
                ...
                output_with_id[index : index + 1] = [movieId + len(self.vocab)] * len(
                    tokenized_movie
                )
    ...
    def map_redial_for_recommender(self, example):
        ...
        movieIds = [
            self.db2id[dbId]
            for dbId in example["movieIds"]
            if len(self.db2name.get(dbId, "")) > 0
        ]
        movie_occurrences = [
            self._fill_movie_occurrences(encoding, texts, movie_name=self.id2name[id])
            for id in movieIds
        ]
        dialogue = []
        target = []
        ...
        for txt in example["messages"]:
            ...
            message_text = pattern.sub(
                lambda m: " " + m.group(1) + " ", txt
            )  # e.g. replace @2019 with 2019
        ...
        # tokenize for rnn
        dialogue = pad_and_stack(
            [torch.tensor([self.token2id(token) for token in s]) for s in dialogue]
        )
        target = pad_and_stack(
            [torch.tensor([self.token2id(token) for token in s]) for s in target]
        )
    ...
    def map_redial_for_sentiment_analysis(self, example):
        ...
        # fill movie_occurences after tokenization
        movie_occurrences = self._fill_movie_occurrences(
            encoding, texts, movie_name=self.db2name[example["movieId"]]
        )
