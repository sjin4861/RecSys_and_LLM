...
from recwizard.utility import init_deterministic, loadJsonFileFromDataset, pad_and_stack
    ...
    def evaluation_loop(
        ...
        for batch in tqdm(dataloader):
            ...
            with torch.no_grad():
                ...
                batch_size, max_conv_length, max_seq_length, vocab_size = (
                    outputs.data.shape
                )
                # indices of recommender's utterances(< batch * max_conv_len)
                idx = torch.nonzero((input["senders"].view(-1) == -1).data).squeeze()
                # select recommender's utterances for the loss
                outputs = outputs.view(-1, max_seq_length, vocab_size).index_select(
                    0, idx
                )
                target = target.view(-1, max_seq_length).index_select(0, idx)
            ...
            recommendation_idx = (target.view(-1) >= self.num_vocab) & (
                ...
                != torch.cat(
                    [
                        torch.zeros(target.shape[0], 1, dtype=torch.long).to(device)
                        - 1,
                        target[:, :-1],
                    ],
                    dim=1,
            ...
            recommendation_outputs = outputs.view(-1, vocab_size)[recommendation_idx][
                :, self.num_vocab :
            ]
            ...
            for b in range(recommendation_target.shape[0]):
                recalls["1"].append(
                    hit(recommendation_target[b].item(), pred_idx[b][:1].tolist())
                )
                recalls["10"].append(
                    hit(recommendation_target[b].item(), pred_idx[b][:10].tolist())
                )
                recalls["50"].append(
                    hit(recommendation_target[b].item(), pred_idx[b][:50].tolist())
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--autorec_model",
        type=str,
        default="save/redial/autorec/db_model_best/pytorch_model.bin",
    )
    parser.add_argument(
        "--sa_model", type=str, default="save/redial/sa/model_best/pytorch_model.bin"
    )
    ...
    tokenized_datasets = raw_datasets.map(
        dp.map_redial_for_recommender, load_from_cache_file=False
    )
