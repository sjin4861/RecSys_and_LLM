...
from recwizard.utility import DeviceManager, init_deterministic
...
class DataCollatorForMovielens:
    ...
    def __init__(
        self, train_ratings, random_noise=True, binary_rating=True, max_num_inputs=1e10
    ):
    ...
    def __call__(
        ...
        """
        input, target = torch.zeros((len(batch), self.n_movies)), -torch.ones(
            (len(batch), self.n_movies)
        )
        for i, example in enumerate(batch):
            ...
            for movieId, rating in zip(movieIds, ratings):
                target[i, self.id2index[movieId]] = process_rating(
                    rating, binary_rating=self.binary_rating
                )
...
class RedialTrainer(Trainer):
    ...
    def get_test_dataloader(self, test_dataset: Dataset) -> DataLoader:
        ...
        return DataLoader(
            ...
            collate_fn=DataCollatorForRedial(
                subset="eval", random_noise=False, db2id=self.db2id
            ),
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        return DataLoader(
            ...
            collate_fn=DataCollatorForRedial(
                subset="train", random_noise=True, db2id=self.db2id
            ),
            drop_last=self.args.dataloader_drop_last,
            num_workers=self.args.dataloader_num_workers,
            pin_memory=self.args.dataloader_pin_memory,
    ...
    def evaluation_loop(
        ...
        metrics = {
            f"{metric_key_prefix}_loss": np.sqrt(
                self.criterion.normalize_loss_reset(np.mean(losses))
            )
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--random_noise", "-r", action="store_true", help="Enable random_noise loading"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1e10,
        help="ax number of inputs (for random_noise loading mode)",
    )
    ...
    if args.data in ["movielens", "db_pretrain"]:
        ...
        train_ratings = {
            ex["userId"]: (ex["movieIds"], ex["ratings"])
            for ex in tqdm(datasets["train"])
        }
        print("Selecting data from userId in training set")
        for subset in subsets[1:]:
            datasets[subset] = datasets[subset].filter(
                lambda x: x["userId"] in train_ratings
            )
    ...
    if args.data in ["db", "db_pretrain"]:
        ...
        model.load_checkpoint(
            os.path.join(
                args.mloutput_dir, "pretrained_model_best", "pytorch_model.bin"
            )
        )
