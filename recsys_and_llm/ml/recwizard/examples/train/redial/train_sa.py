...
from recwizard.modules.redial.hrnn_for_classification import (
    HRNNForClassification,
    RedialSentimentAnalysisLoss,
)
from recwizard.modules.redial.params import sentiment_analysis_params
from recwizard.utility import DeviceManager, init_deterministic, pad_and_stack
    ...
    def __call__(
        ...
        res = {
            "input_ids": pad_and_stack(
                [ex["input_ids"] for ex in batch], pad_value=self.pad_token_id
            ),
            "attention_mask": pad_and_stack([ex["attention_mask"] for ex in batch]),
            "movie_occurrences": pad_and_stack(
                [ex["movie_occurrences"] for ex in batch]
            ),
            ...
            "conversation_lengths": torch.tensor(
                [ex["conversation_lengths"] for ex in batch]
            ),
        }
        return res
...
class RedialSATrainer(Trainer):
    ...
    def evaluation_loop(
        ...
        Iconfusion_matrix_no_disagreement = np.zeros(
            (matrix_size, matrix_size), dtype=int
        )
        Rconfusion_matrix_no_disagreement = np.zeros(
            (matrix_size, matrix_size), dtype=int
        )
        wrongs = 0
        ...
        for batch in tqdm(dataloader):
            ...
            wrongs += np.sum(
                1 * (Iliked.data.numpy() != target[:, 2])
                + 1 * (Rliked.data.numpy() != target[:, 5])
            )
            ...
            wrongs_with_disagreement += np.sum(
                1
                * (Iliked.data.numpy() != target[:, 2])
                * (target[:, 2] != target[:, 5])
                + 1
                * (Rliked.data.numpy() != target[:, 5])
                * (target[:, 2] != target[:, 5])
            )
        ...
        if metric_key_prefix == "test":
            ...
            Iliked_marginal_no_disagreement = Iconfusion_matrix_no_disagreement.reshape(
                3, 3, 2, 3, 3, 2
            ).sum(axis=(1, 2, 4, 5))
            Rliked_marginal_no_disagreement = Rconfusion_matrix_no_disagreement.reshape(
                3, 3, 2, 3, 3, 2
            ).sum(axis=(1, 2, 4, 5))
            print("marginals")
            print(I_marginal)
        ...
        output = EvalLoopOutput(
            predictions=None, label_ids=None, metrics=metrics, num_samples=total
        )
        return output
...
if __name__ == "__main__":
    ...
    tokenized_datasets = raw_datasets.map(
        preprocessor.map_redial_for_sentiment_analysis
    )
    ...
    trainer = RedialSATrainer(
        ...
        criterion=RedialSentimentAnalysisLoss(
            class_weight={"liked": [1.0 / 5, 1.0 / 80, 1.0 / 15]},
            use_targets="suggested seen liked",
        ).to(device),
