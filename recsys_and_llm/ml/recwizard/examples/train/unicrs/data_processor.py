...
class KGDataLoader:
    ...
    def get_entity_kg_info(cls, dataset):
        ...
        if not os.path.exists(dataset_dir):
            snapshot_download(
                f"{HF_ORG}/{dataset}", repo_type="dataset", local_dir=dataset_dir
            )
        with open(
        ...
        with open(
            os.path.join(dataset_dir, "entity2id.json"), "r", encoding="utf-8"
        ) as f:  # only need len
        ...
        with open(
            os.path.join(dataset_dir, "item_ids.json"), "r", encoding="utf-8"
        ) as f:  # extracted
...
class UnicrsDataProcessor:
    ...
    def prepare_data_for_pretrain(self, batched_dialog):
        ...
        for dialog in batched_dialog.pa_table.to_pylist():
            ...
            entities = [
                self.entity2id[m.group(1)]
                for m in self.entity_pattern.finditer(resp)
                if m.group(1) in self.entity2id
            ]
    ...
    def prepare_data_for_rec(
        self, batched_dialog, indices, gen_data=None, use_resp=False
    ):
        """
...
if __name__ == "__main__":
    ...
    print(
        ...
        {
            k: word_cnt[k]
            for k in sorted(word_cnt.keys(), key=lambda x: -word_cnt[x])[:10]
        },
    )
