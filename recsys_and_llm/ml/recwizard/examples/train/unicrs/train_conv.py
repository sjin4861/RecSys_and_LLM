...
import numpy as np
import torch
from data_processor import KGDataLoader, UnicrsDataProcessor
from datasets import load_dataset
from evaluator import ConvEvaluator
from recwizard.modules.unicrs import (
    UnicrsGen,
    UnicrsGenConfig,
    UnicrsGenTokenizer,
    UnicrsRecConfig,
...
class CRSDataCollatorForConv:
    ...
    def __call__(self, data_batch):
        ...
        if sum(map(len, batch["resp"])) == 0:  # training
            ...
            labels = self.tokenizer(batch["resp"], add_special_tokens=False)["context"][
                "input_ids"
            ]
...
def parse_args():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="save/unicrs_conv/redial",
    ...
    # model
    parser.add_argument(
        "--pretrained_model", type=str, default="save/unicrs_pre/redial/model_best"
    )
    parser.add_argument(
        "--model",
        type=str,
...
if __name__ == "__main__":
    ...
    conv_dataset = datasets.map(
        dp.prepare_data_for_conv, fn_kwargs={"gen": False}, load_from_cache_file=False
    )
    gen_dataset = datasets.map(
        dp.prepare_data_for_conv, fn_kwargs={"gen": True}, load_from_cache_file=False
    )

    kgprompt_config = UnicrsRecConfig.from_pretrained(
    ...
    model.load_checkpoint(
        os.path.join(args.pretrained_model, STATE_DICT_FILE), strict=False
    )
    ...
    num_update_steps_per_epoch = math.ceil(
        len(datasets["train"]) / args.batch_size / args.gradient_accumulation_steps
    )
    ...
    if args.run_infer:
        ...
        for subset in gen_dataset:
            ...
            trainer.evaluator.load_log_file(
                "{}/{}_gen.jsonl".format(output_dir, subset)
            )
