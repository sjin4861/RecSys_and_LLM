...
class RecEvaluator:
    ...
    def evaluate(self, logits, labels):
        ...
        if self.item_ids:
            ranks = [
                [self.item_ids[rank] for rank in batch_rank] for batch_rank in ranks
            ]
    ...
    def evaluate(self, preds, labels, log=False):
        ...
        if log and hasattr(self, "log_file"):
            for pred, label in zip(decoded_preds, decoded_labels):
                self.log_file.write(json.dumps({"pred": pred, "label": label}) + "\n")
