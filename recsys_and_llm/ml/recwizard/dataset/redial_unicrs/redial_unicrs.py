        ...
        if self.config.name == "unrolled":
            ...
            with open(filepath, encoding="utf-8") as f:
                for line in f:
                    ...
                    user_id, resp_id = (
                        dialog["initiatorWorkerId"],
                        dialog["respondentWorkerId"],
                    )
                    context, resp = [], ""
                    ...
                    while turn_i < len(messages):
                        ...
                        while (
                            turn_j < len(messages)
                            and messages[turn_j]["senderWorkerId"] == worker_id
                        ):
                            utt = self._process_utt(
                                messages[turn_j]["text"],
                                movieid2name,
                                replace_movieId=True,
                            )
                            utt = markup_entity(
                                utt,
                                messages[turn_j]["entity_name"]
                                + messages[turn_j]["movie_name"],
                            )
                            utt_turn.append(utt)
                            movie_ids = [
                                entity2id[movie]
