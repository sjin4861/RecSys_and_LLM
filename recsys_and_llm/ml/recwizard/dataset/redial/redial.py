    ...
    BUILDER_CONFIGS = [
        RedialConfig(
            ...
            features={
                ...
                "form": datasets.features.Sequence(datasets.Value("int32"), length=6),
            },
    ...
    def _flattenMessages(self, conversation, add_prefix=False):
        ...
        for message in conversation["messages"]:
            ...
            if len(senders) > 0 and senders[-1] == role:
                ...
                if add_prefix:
                    prefix = (
                        self.config.initiator_prefix
                        if role == 1
                        else self.config.respondent_prefix
                    )
                    text = prefix + text
        ...
        if self.config.name == "autorec":
            with open(filepath, encoding="utf-8") as f:
                ...
                for line in f:
                    ...
                    if len(movieIds) > 0:
                        yield idx, {"movieIds": movieIds, "ratings": ratings}
                        idx += 1
            ...
            with open(filepath, encoding="utf-8") as f:
                for line in f:
                    ...
                    for id in gen:
                        ...
                        movieName = date_pattern.sub(
                            "", conversation["movieMentions"][id]
                        ).strip(" ")
                        if len(movieName) == 0:
                            continue
                        yield Idx, {
            ...
            with open(filepath, encoding="utf-8") as f:
                for line in f:
                    ...
                    yield Idx, {
                        ...
                        "senders": senders,
                        "movieIds": [
                            int(movieId) for movieId in conversation["movieMentions"]
                        ],
                    }
                    Idx += 1
            ...
            with open(filepath, encoding="utf-8") as f:
                for line in f:
                    ...
                    formatted_msgs = [
                        self._process_utt(utt, movieid2name) for utt in msgs
                    ]
                    yield Idx, {
