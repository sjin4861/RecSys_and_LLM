...
def load_movie_mappings(path):
    ...
    db2name = {
        db: date_pattern.sub("", id2name[id]).strip(" ") for db, id in db2id.items()
    }
...
def get_vocab(dataset, db2name):
    ...
    print(
        "Vocab covers {} word instances over {}".format(
...
if __name__ == "__main__":
    ...
    id2entity, db2name = load_movie_mappings(
        os.path.join(base_dir, "movies_merged.csv")
    )
