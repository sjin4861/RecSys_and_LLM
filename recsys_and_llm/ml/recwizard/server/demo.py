...
from utils import create_graph, img_to_html, plotly_plot, plotly_timeline, strip_debug
...
model_name = st.selectbox(
    label="Select Model",
    options=requests.get(f"http://localhost:{PORT}/listmodels").json(),
)
st.subheader(model_name, divider="rainbow")
with st.spinner("Loading model..."):
    model_loading_status = requests.get(
        f"http://localhost:{PORT}/loadmodel", {"model_name": model_name}
    ).json()
...
if model_loading_status == "failure":
    ...
    gen_args = st.sidebar.text_input(
        "gen_args (dict)", value="{}", on_change=dict_validator, key="gen_args"
    )
    rec_args = st.sidebar.text_input(
        "rec_args (dict)", value="{}", on_change=dict_validator, key="rec_args"
    )
    if "chatgpt" in model_name.lower():
        chatgpt_model = st.sidebar.radio(
            label="ChatGPT model", options=["gpt-4", "gpt-3.5"]
        )
    ...
    for i in st.session_state["chat_history"][model_name]:
        with st.chat_message(
            name=i["role"], avatar="recwizard.png" if i["role"] == "assistant" else None
        ):
            if "content" in i:
                st.write(i["content"])
