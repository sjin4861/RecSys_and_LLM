...
class ChatgptAgent(BasePipeline):
    ...
    def response(self, query, **kwargs):
        ...
        rec = self.rec_module.response(
            gen_output, topk=n_movies, tokenizer=self.rec_tokenizer
        )

        pure_output = gen_output.split("System: ")[1]
        input = 'Please replace the "<{}>" in the text "{}" by the following phrases "{}" respectively. '.format(
            self.answer_type, pure_output, rec
        )
        input += self.prompt
