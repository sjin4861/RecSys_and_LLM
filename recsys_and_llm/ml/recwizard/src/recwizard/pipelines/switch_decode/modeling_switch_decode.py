...
class SwitchDecodePipeline(BasePipeline):
    ...
    def __init__(self, config, temperature=1, sample_movies=False, **kwargs):
        ...
        self.switch = nn.Linear(
            in_features=self.config.hidden_size + self.config.context_size,
            out_features=1,
        )
    ...
    def switch_decode(
        ...
        if self.sample_movies:
            ...
            movie_recommendations = torch.zeros(
                batch_size, max_utterance_length, n_movies, device=self.device
            )
            ...
            output = torch.cat(
                (
                    F.logsigmoid(switch)
                    + F.log_softmax(language_output / temperature, dim=-1),
                    F.logsigmoid(-switch)
                    + F.log_softmax(movie_recommendations / temperature, dim=-1),
                ),
                dim=2,
            )
        return output
    ...
    def forward(self, rec_inputs, gen_inputs, forbid_movies):
        batch_size, max_conv_length, max_utterance_length = gen_inputs[
            "dialogue"
        ].shape[:3]
        recs = (
            self.recs
            if self.recs is not None
            else self.rec_module.forward(**rec_inputs)
        )  # (batch, seq_len, n_movies)
        pad_tensor = torch.zeros(batch_size, 1, recs.shape[-1], device=recs.device)
        recs = torch.cat((pad_tensor, recs), 1)[:, -2, :]
        context_, hidden, language_output = self.gen_module.forward(
            **gen_inputs, context=self.context
        )
        ...
        context = context.expand(
            batch_size * max_conv_length, max_utterance_length, self.context_size
        ).contiguous()
        ...
        language_output = language_output.view(
            batch_size * max_conv_length, max_utterance_length, -1
        )
        output = self.switch_decode(
            switch_input,
            language_output,
            recs,
            temperature=self.t,
    ...
    def replace_movie_with_words(self, tokens):
        ...
        for token_id in tokens:
            if token_id <= self.vocab_size:
                ...
                res.extend(
                    self.gen_tokenizer.encode(movie_name, add_special_tokens=False)
                )
        return res

    def response(
        self, query: str, beam_size=10, forbid_movies=None, temperature=1, **kwargs
    ):
        ...
        conv_repr = self.gen_module.response(
            query, self.gen_tokenizer, encoder_only=True
        )
        ...
        for i in range(self.config.max_seq_length):
            ...
            for beam in beams:
                # add batch_dimension
                gen_inputs = {
                    "dialogue": torch.tensor(
                        beam.sequence, device=self.device, dtype=torch.long
                    )
                    .unsqueeze(0)
                    .unsqueeze(0),
