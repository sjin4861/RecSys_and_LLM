...
class UnicrsRecTokenizer(BaseTokenizer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        context_tokenizer = AutoTokenizer.from_pretrained(
            context_tokenizer, truncation_side="left"
        )
        prompt_tokenizer = AutoTokenizer.from_pretrained(
            prompt_tokenizer, truncation_side="left"
        )
        ...
        tokenizers = [context_tokenizer, prompt_tokenizer]
        super().__init__(
    ...
    def load_from_dataset(cls, dataset="redial_unicrs", **kwargs):
        ...
        return cls(
            entity2id=entityName2id, pad_entity_id=max(entity2id.values()) + 1, **kwargs
        )
    ...
    def mergeEncoding(encodings: List[BatchEncoding]) -> BatchEncoding:
        ...
        if len(encodings) == 0:
            return res
        res.data = {"context": encodings[0], "prompt": encodings[1]}
        return res
