...
from recwizard.utility import SEP_TOKEN, Singleton, deterministic_seed
...
class UnicrsGen(BaseModule):
    ...
    def __init__(
        self, config: UnicrsGenConfig, edge_index=None, edge_type=None, **kwargs
    ):
        super().__init__(config, **kwargs)
        edge_index = self.prepare_weight(edge_index, "edge_index")
        edge_type = self.prepare_weight(edge_type, "edge_type")
        self.encoder = KGPrompt(
            **config.kgprompt_config, edge_index=edge_index, edge_type=edge_type
        )
        self.decoder = Singleton(
            "unicrs.PromptGPT2",
            PromptGPT2LMHead.from_pretrained(config.pretrained_model),
        )
    ...
    def forward(self, context, prompt, entities, labels, **kwargs):
        prompt_embeds = self.encoder(
            entity_ids=entities, prompt=prompt, rec_mode=False, use_conv_prefix=True
        )
    ...
    def response(self, raw_input: str, tokenizer, return_dict=False):
        ...
        text = decoded_text.replace(
            str(tokenizer.tokenizers[0].eos_token), "\n"
        ).strip()
