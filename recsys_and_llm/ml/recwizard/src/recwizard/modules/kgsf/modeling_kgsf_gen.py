...
from .transformer_utils import TransformerDecoderKG, TransformerEncoder
from .utils import (
    _create_embeddings,
...
class KGSFGen(BaseModule):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.representation_bias = nn.Linear(
            config.embedding_size, len(self.dictionary) + 4
        )
        ...
        self.concept_edge_sets = torch.LongTensor(config.edge_set).to(
            device
        )  # _concept_edge_list4GCN(config)#
    ...
    def decode_greedy(
        ...
        for i in range(maxlen):
            scores = self.decoder(
                xs, encoder_states, encoder_states_kg, encoder_states_db
            )
            scores = scores[:, -1:, :]
