...
class KGSFRec(BaseModule):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.copy_norm = nn.Linear(
            config.embedding_size * 2 + config.embedding_size, config.embedding_size
        )
        ...
        self.dbpedia_RGCN = RGCNConv(
            config.n_entity, self.dim, self.n_relation, num_bases=config.num_bases
        )
    ...
    def infomax_loss(self, db_nodes_features, con_user_emb, db_label, mask):
        ...
        info_db_loss = torch.sum(
            self.info_db_loss(db_scores, db_label.to(device).float()), dim=-1
        ) * mask.to(device)
    ...
    def forward(
        ...
        # graph network
        db_nodes_features = self.dbpedia_RGCN(
            None, self.db_edge_idx, self.db_edge_type
        )  # entity_encoder
        ...
        if response is not None:
            info_db_loss = self.infomax_loss(
                db_nodes_features, con_user_emb, db_vec, db_con_mask
            )
            rec_loss = self.criterion(
                entity_scores.squeeze(1).squeeze(1).float(), labels.to(device)
            )
        ...
        result = {
            "loss": loss,
            "labels": labels,
            "rec_scores": torch.tensor(entity_scores),
            "rec_loss": rec_loss,
        }
        print(result, labels)
        return result
    ...
    @monitor
    def response(
        self, raw_input: Union[List[str], str], tokenizer, return_dict=False, topk=3
    ):
        movieIds_lst = []
        movieNames_lst = []
