...
class KGSFRecTokenizer(BaseTokenizer):
    ...
    def encode(
        self, user_input, user_context=None, entity=None, system_response=None, movie=0
    ):
        """
    ...
    def decode(self, outputs, top_k=3, labels=None):
        outputs = outputs[
            :, torch.LongTensor(self.entity_ids)
        ]  # previous outputs.shape=(1,64368), now 6924, same as len(entity_ids)
