...
from recwizard.utility import DeviceManager, sort_for_packed_sequence
...
class RedialGen(BaseModule):
    ...
    def __init__(self, config: RedialGenConfig, word_embedding=None, **kwargs):
        ...
        word_embedding = nn.Embedding.from_pretrained(
            self.prepare_weight(word_embedding, "decoder.embedding")
        )
        DeviceManager.device = self.device
        self.decoder = SwitchingDecoder(
            word_embedding=word_embedding, **config.decoder_params
        ).to(self.device)
    ...
    def forward(
        ...
        **hrnn_input,
        ...
        conv_repr = (
            context if context is not None else self.encoder(**hrnn_input)["conv_repr"]
        )  # (batch, hidden_size)
        if encode_only:
            return conv_repr
        (
        ...
        ) = self.prepare_input_for_decoder(dialogue, lengths, conv_repr, recs)
        ...
        outputs = outputs.index_select(0, rev).view(
            batch_size, max_conv_length, max_utterance_length, -1
        )
    ...
    def response(
        ...
        **kwargs,
        ...
        gen_input = DeviceManager.copy_to_device(
            tokenizer([raw_input]).data, device=self.device
        )
    ...
    def prepare_input_for_decoder(self, dialogue, lengths, conv_repr, recs):
        ...
        pad_tensor = torch.zeros(
            batch_size,
            1,
            int(self.config.hrnn_params["conversation_encoder_hidden_size"]),
            device=self.device,
        )
        ...
        conv_repr = (
            ...
            .view(
                batch_size * max_conv_length,
                self.config.hrnn_params["conversation_encoder_hidden_size"],
            )
        ...
        recs = (
            recs.contiguous()
            .view(batch_size * max_conv_length, -1)
            .index_select(0, sorted_idx)
        )
...
class DecoderGRU(nn.Module):
    ...
    def forward(self, input_sequence, lengths, context=None, state=None):
        ...
        if context is not None:
            ...
            if self.peephole:
                context_for_input = context.unsqueeze(1).expand(
...
class SwitchingDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.language_out = nn.Linear(
            in_features=self.hidden_size, out_features=self.vocab_size
        )
        self.switch = nn.Linear(
            in_features=self.hidden_size + self.context_size, out_features=1
        )
    ...
    def forward(
        ...
        movie_recommendations = (
            movie_recommendations.unsqueeze(1)
            .expand(batch_size, max_utterance_length, n_movies)
            .contiguous()
        )
        ...
        if sample_movies:
            ...
            movie_probabilities = F.softmax(
                movie_recommendations.view(-1, n_movies), dim=1
            )
            # zero out the forbidden movies
            if forbid_movies is not None:
                if batch_size > 1:
                    raise ValueError(
                        "forbid_movies functionality only implemented with batch_size=1 for now"
                    )
                for movieId in forbid_movies:
                    movie_probabilities[:, movieId] = 0
            # Sample movies
            sampled_movies = (
                movie_probabilities.multinomial(1)
                .view(batch_size, max_utterance_length)
                .data.cpu()
                .numpy()
            )
            # Fill a new recommendations vector with sampled movies
            movie_recommendations = torch.zeros(
                batch_size, max_utterance_length, n_movies, device=DeviceManager.device
            )
            for i in range(batch_size):
                for j in range(max_utterance_length):
                    ...
                    movie_recommendations[i, j, sampled_movies[i, j]] = (
                        max_probabilities[i]
                    )

            if log_probabilities:
                raise ValueError(
                    "Sample movies only works with log_probabilities=False for now."
                )
                # output = torch.cat((
                ...
                output = torch.cat(
                    (
                        switch.sigmoid()
                        * F.softmax(language_output / temperature, dim=2),
        ...
        if log_probabilities:
            output = torch.cat(
                (
                    F.logsigmoid(switch)
                    + F.log_softmax(language_output / temperature, dim=2),
                    F.logsigmoid(-switch)
                    + F.log_softmax(movie_recommendations / temperature, dim=2),
                ),
                dim=2,
            )
        else:
            output = torch.cat(
                (
                    ...
                    (-switch).sigmoid()
                    * F.softmax(movie_recommendations / temperature, dim=2),
                ),
                dim=2,
            )
        return output
    ...
    def replace_movie_with_words(self, tokens, tokenizer):
        ...
        for token_id in tokens:
            if token_id <= self.vocab_size:
                ...
                res.extend(
                    tokenizer.tokenizers[1].encode(movie_name, add_special_tokens=False)
                )
        return res
        ...
        for i in range(max_seq_length):
            ...
            for beam in beams:
                ...
                model_input = torch.tensor(
                    beam.sequence, device=DeviceManager.device, dtype=torch.long
                ).unsqueeze(0)
                beam_forbidden_movies = forbid_movies.union(beam.mentioned_movies)
                prob = self.forward(
                    input=model_input,
                    lengths=torch.tensor(
                        [len(beam.sequence)], device=DeviceManager.device
                    ),
            ...
            for beam in beams:
                if beam.sequence[-1] > self.vocab_size:
                    ...
                    beam.sequence[-1:] = self.replace_movie_with_words(
                        beam.sequence[-1:], tokenizer
                    )
