...
class HRNNForClassification(BaseModule):
    ...
    def __init__(
        ...
        self.linears = nn.ModuleDict(
            {
                output: nn.Linear(
                    (1 + conv_bidrectional) * encoder_output_size, num_classes
                )
    ...
    def forward(
        ...
        if self.multiple_items_per_example:
            ...
            indices = [
                (i, j)  # i-th conversation in the batch, j-th item in the conversation
                for (i, conv_item_occurrences) in enumerate(movie_occurrences)
                for j in range(len(conv_item_occurrences))
            ]
            batch_indices = torch.tensor(
                [i[0] for i in indices], device=input_ids.device
            )
            ...
            movie_occurrences = pad_and_stack(
                [oc for conv_oc in movie_occurrences for oc in conv_oc]
            )
        ...
        if self.return_liked_probability:
            ...
            output = {
                "i_liked": F.softmax(self.linears["i_liked"](conv_repr), dim=-1)[
                    :, :, 1
                ]
            }
        ...
        if (
            self.multiple_items_per_example
        ):  # recover the batch shape from flattend output
            batch_output = {
                key: [[] for _ in range(batch_size)] for key in output.keys()
            }
...
class RedialSentimentAnalysisLoss(nn.Module):
    def __init__(self, class_weight, use_targets):
        ...
        if self.class_weight and "liked" in self.class_weight:
            self.liked_criterion = nn.CrossEntropyLoss(
                weight=torch.Tensor(self.class_weight["liked"])
            )
        else:
            self.liked_criterion = nn.CrossEntropyLoss()
    ...
    def forward(self, output, target):
        ...
        if "suggested" in self.use_targets:
            loss += self.weights[0] * (
                self.suggested_criterion(
                    output["i_suggested"].squeeze(1), target[:, 0].float()
                )
                + self.suggested_criterion(
                    output["r_suggested"].squeeze(1), target[:, 3].float()
                )
            )
        if "seen" in self.use_targets:
            loss += self.weights[1] * (
                self.seen_criterion(output["i_seen"], target[:, 1])
                + self.seen_criterion(output["r_seen"], target[:, 4])
            )
        if "liked" in self.use_targets:
            loss += self.weights[2] * (
                self.liked_criterion(output["i_liked"], target[:, 2])
