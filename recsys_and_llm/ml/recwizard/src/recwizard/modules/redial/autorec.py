...
class AutoRec(
    BaseModule
):  # We use features from BaseModule to load previous checkpoints
    """
    ...
    def __init__(self, n_movies, layer_sizes, g, f):
        ...
        if g == "identity":
            ...
            raise ValueError(
                "Got invalid function name for g : {}".format(self.config.g)
            )

        self.encoder = UserEncoder(
            layer_sizes=self.layer_sizes, n_movies=self.n_movies, f=f
        )
        self.user_representation_size = self.layer_sizes[-1]
        self.decoder = nn.Linear(
            in_features=self.user_representation_size, out_features=self.n_movies
        )
        ...
        if encoder0weight.shape[1] < self.n_movies:
            encoder0weight = torch.cat(
                (
                    encoder0weight,
                    torch.zeros(
                        encoder0weight.shape[0],
                        self.n_movies - encoder0weight.shape[1],
                        device=self.device,
                    ),
                ),
                dim=1,
            ...
            decoderweight = torch.cat(
                (
                    decoderweight,
                    torch.zeros(
                        self.n_movies - decoderweight.shape[0],
                        decoderweight.shape[1],
                        device=self.device,
                    ),
                ),
                dim=0,
            ...
            decoderbias = torch.cat(
                (
                    decoderbias,
                    torch.zeros(
                        self.n_movies - decoderbias.shape[0], device=self.device
                    ),
                ),
                dim=0,
            )
...
class UserEncoder(nn.Module):
    def __init__(self, layer_sizes, n_movies, f):
        ...
        self.layers = nn.ModuleList(
            [
                (
                    nn.Linear(in_features=n_movies, out_features=layer_sizes[0])
                    if i == 0
                    else nn.Linear(
                        in_features=layer_sizes[i - 1], out_features=layer_sizes[i]
                    )
                )
