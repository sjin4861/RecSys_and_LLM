...
class Beam(object):
    ...
    def __str__(self):
        ...
        return finished_str + " finished beam of likelihood {} : {}".format(
            self.likelihood, self.sequence
        )
...
class BeamSearch(object):
    ...
    def update_beams(beams, beam_size, probabilities, n_gram_block=None):
        ...
        new_probabilities = np.array(
            [
                (
                    beam.likelihood * probability.data.numpy()
        ...
        ind = np.unravel_index(
            np.argsort(new_probabilities, axis=None), new_probabilities.shape
        )
        ...
        for beam_index, token in zip(*ind):
            ...
            # otherwise, update the beam with the chosen token
                ...
                if n_gram_block is None or not n_gram_repeats(
                    beams[beam_index].sequence + [token], n_gram_block
                ):
                    # add extended hypothesis to new_beam list
                    new_beams.append(
                        beams[beam_index].get_updated_beam(
                            token, probabilities[beam_index][token].data.numpy()
                        )
                    )
