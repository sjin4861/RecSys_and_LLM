...
from recwizard.utility import DeviceManager, WrapSingleInput
...
class RedialRec(BaseModule):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if movieIds.numel() > 0:
            ...
            i_liked = torch.zeros(
                input_ids.shape[0], 0, self.n_movies, device=self.device
            )
        ...
        autorec_input = torch.zeros(
            batch_size, max_conv_length, self.n_movies, device=self.device
        )
        for i in range(batch_size):
            for j in range(len(movieIds[i])):
                mask = movie_occurrences[i][j].sum(dim=1).cumsum(dim=0) > 0
                autorec_input[i, : len(mask), movieIds[i][j]] = (
                    mask * i_liked[i][j][: len(mask)]
                )
    ...
    def response(self, raw_input, tokenizer, return_dict=False, topk=3, **kwargs):
        tokenized_input = DeviceManager.copy_to_device(
            tokenizer(raw_input).data, device=self.device
        )
        logits = self.forward(**tokenized_input)
        movieIds = (
            logits.topk(k=topk, dim=-1).indices[:, -1, :].tolist()
        )  # selects the recommendation for the last sentence
