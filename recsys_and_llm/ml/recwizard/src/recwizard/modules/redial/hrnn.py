...
class HRNN(nn.Module):
    ...
    def __init__(
        ...
        self.conversation_encoder = nn.GRU(
            ...
            + 1
            + (use_movie_occurrences == "sentence"),
    ...
    def get_sentence_representations(
        self, input_ids, attention_mask, senders, movie_occurrences=None
    ):
        ...
        with torch.no_grad():
            embedded = self.base_encoder(
                **batch_encoding, output_hidden_states=True, return_dict=True
            ).last_hidden_state
        ...
        )  # (num_positive_lengths , 2*hidden_size)
        ...
        if num_positive_lengths < batch_size * max_conversation_length:
            ...
            sentence_representations = torch.cat(
                (sentence_representations, pad_tensor), 0
            )
        ...
        sentence_representations = sentence_representations.index_select(0, rev).view(
            batch_size, max_conversation_length, 2 * self.sentence_encoder_hidden_size
        )
        # Append sender information
        sentence_representations = torch.cat(
            [sentence_representations, senders.unsqueeze(2)], 2
        )
        ...
        if self.use_movie_occurrences == "sentence":
            ...
            sentence_representations = torch.cat(
                (
                    ...
                    torch.sign(torch.sum(movie_occurrences, dim=-1, keepdim=True)),
                ),
                2,
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        sentence_representations = self.get_sentence_representations(
            input_ids, attention_mask, senders, movie_occurrences
        )
        ...
        conversation_representations, last_state = self.conversation_encoder(
            packed_sequences
        )
