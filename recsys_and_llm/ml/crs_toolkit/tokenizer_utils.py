...
import torch
from crs_toolkit.utility import (
    BOS_TOKEN,
    ENTITY_PATTERN,
    EOS_TOKEN,
    SEP_TOKEN,
...
class BaseTokenizer(PreTrainedTokenizer):
    ...
    def __init__(
        ...
        tokenizers: Union[
            List[PreTrainedTokenizerBase], PreTrainedTokenizerBase
        ] = None,
        **kwargs,
    ):
        """
        ...
        if self.entity2id is not None:
            self.pad_entity_id = (
                pad_entity_id
                if pad_entity_id is not None
                else max(self.entity2id.values()) + 1
            )
    ...
    def batch_encode_plus(
        ...
        if self.entity2id:
            ...
            if self.tokenizers is None:
                return BatchEncoding(
                    {"raw_text": processed_text, "entities": batch_entities}
                )
        else:
    ...
    def encode_plus(self, text: str, *args, **kwargs) -> BatchEncoding:
        ...
        if self.tokenizers is None:
            return BatchEncoding({"raw_text": processed_text, "entities": entities})
        texts = self.replace_special_tokens(processed_text)
        encodings = self.encodes(
            [tokenizer.encode_plus for tokenizer in self.tokenizers],
            texts,
            *args,
            **kwargs,
        )
    ...
    def get_init_kwargs(self):
        ...
        """
        return {"entity2id": self.entity2id, "pad_entity_id": self.pad_entity_id}
    ...
    def save_vocabulary(
        ...
        vocab_path = os.path.join(
            save_directory, filename_prefix, self.init_kwargs_file
        )
